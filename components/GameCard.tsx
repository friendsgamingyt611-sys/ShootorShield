...
export const GameCard: React.FC<GameCardProps> = ({ stats, isPlayer, lastAction, damageTaken, penaltyTaken, compact = false }) => {
  ...
  // Haptic Feedback & Damage Shake
  useEffect(() => {
      if ((damageTaken && damageTaken > 0) || (penaltyTaken && penaltyTaken > 0)) {
          // Haptic Feedback
          if (navigator.vibrate) {
              navigator.vibrate(isPlayer ? [50, 50, 50] : [30]);
          }
      }
  }, [damageTaken, penaltyTaken, isPlayer]);
  ...
  // Intensified shake for damage
  const shakeClass = hasDamage ? 'animate-shake' : '';
  const flashClass = flash ? 'animate-flash-upgrade' : '';
  const impactClass = hasDamage ? 'filter drop-shadow-[0_0_25px_rgba(220,38,38,1)] brightness-150 saturate-200 contrast-125' : '';
  ...
  const RenderHumanoid = () => {
    const isSpectre = stats.character.id === 'c1'; 
    const isVanguard = stats.character.id === 'c2'; 
    const isNeon = stats.character.id === 'c3'; 
    ...
    const energyColor = isPlayer ? '#3b82f6' : '#ef4444'; 
    ...
    if (isShooting) roleAnim = "animate-recoil"; 
    ...
    const renderGun = () => {
        ...
        return (
          <g>
             <rect x="-4" y="-2" width="8" height="12" fill="#333" />
             ...
             <g transform="translate(0, 0)">
                {isSpectre && (
                  <g>
                     ...
                     <rect x="-8" y="2" width="16" height="4" fill={energyColor} className="animate-pulse" />
                     <path d="M-12 -5 L-12 -15" stroke="#444" strokeWidth="2" />
                  </g>
                )}
                {isVanguard && (
                   <g>
  ...
  return (
    <div className={`${containerClasses} ${shakeClass} ${flashClass} group`}>
      <div className={`${innerClasses} ${compact ? (isPlayer ? 'border-r-2 border-blue-500/10' : 'border-l-2 border-red-500/10') : ''}`}>
        ...
        {/* HIT FLASH OVERLAY - Stronger Impact */}
        {hasDamage && (
             <div className="absolute inset-0 bg-red-500/30 z-50 animate-pulse pointer-events-none mix-blend-color-dodge"></div>
        )}

        {(damageTaken && damageTaken > 0) && (
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-red-500 font-bold text-5xl md:text-7xl animate-bounce z-50 drop-shadow-[0_0_15px_rgba(255,0,0,1)] font-display italic">
            -{damageTaken}
            </div>
        )}
        ...
        <div className={`space-y-1 font-mono relative z-10 w-full ${compact ? 'mt-auto pb-1' : 'bg-black/40 backdrop-blur-sm p-3 rounded-lg border border-white/5 shadow-inner md:space-y-3'}`}>
            <div className="space-y-0.5">
                <div className="w-full bg-gray-900 rounded-sm h-1 md:h-2 overflow-hidden relative border border-gray-800">
                    <div className="bg-gradient-to-r from-red-600 to-red-400 h-full transition-all duration-300" style={{ width: `${healthPct}%` }} />
                </div>
            </div>
            <div className="space-y-0.5">
                <div className="w-full bg-gray-900 rounded-sm h-1 md:h-2 overflow-hidden relative border border-gray-800">
                    <div className="bg-gradient-to-r from-yellow-600 to-yellow-400 h-full transition-all duration-300" style={{ width: `${armorPct}%` }} />
                </div>
            </div>
        </div>
